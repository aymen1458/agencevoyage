...
public class volReservationController {
	...
	@GetMapping("/{id}")
	public volReservation getvolReservation(@PathVariable Long id) {
		return volReservationService.getvolReservationById(id);
	}
	
	
	@DeleteMapping("/{x}")
	public void deletevolReservationById(@PathVariable Long x) {
	 volReservationService.deletevolReservation(x);}
	
	@PostMapping("/{volId}")
	public volReservation addvolReservation(@RequestBody volReservation vr , @PathVariable Long volId) {	

...
public class offreReservationController {
	...
	@GetMapping("/{id}")
	public offreReservation getoffreReservation(@PathVariable Long id) {
		return offreReservationService.getoffreReservationById(id);
	}
	
	
	@DeleteMapping("/{x}")
	public void deleteoffreReservationById(@PathVariable Long x) {
	 offreReservationService.deleteoffreReservation(x);}
	
	@PostMapping("/{offreId}")
	public offreReservation addoffreReservation(@RequestBody offreReservation or , @PathVariable Long offreId) {	
